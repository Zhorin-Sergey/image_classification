...
{ 
  ...
  Ptr<DescriptorExtractor> descExtractor = DescriptorExtractor::create("SIFT");  // создаем объект класса вычисления SIFT дескрипторов 
...
void ComputeImgDescriptor(const string& fileName, Mat& voc, Mat& imgDesc)
{
  ...
  Ptr<DescriptorExtractor> dExtractor = DescriptorExtractor::create("SIFT"); // создаем объект класса вычисления SIFT дескрипторов ключевых точек 
